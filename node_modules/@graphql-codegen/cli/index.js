...
class CodegenContext {
    constructor({ config, graphqlConfig, filepath, }) {
        ...
        this.profiler = pluginHelpers.createNoopProfiler();
    }
    useProject(name) {
        this._project = name;
    }
    ...
    useProfiler() {
        this.profiler = pluginHelpers.createProfiler();
...
async function executeCodegen(input) {
    ...
    const schemaLoadingCache = createCache(async function (hash) {
        ...
        const outputSchema = pluginHelpers.getCachedDocumentNodeFromSchema(outputSchemaAst);
        return {
    ...
    try {
        ...
        if (isListrError(err)) {
            const allErrs = err.errors.map(subErr => pluginHelpers.isDetailedError(subErr)
                ? `${subErr.message} for "${subErr.source}"${subErr.details}`
                : subErr.message || subErr.toString());
            const newErr = new utils.AggregateError(err.errors, `${err.message} ${allErrs.join('\n\n')}`);
...
async function generate(input, saveToFile = true) {
    ...
    if (context.profilerOutput) {
        writeSync(path.join(context.cwd, context.profilerOutput), JSON.stringify(context.profiler.collect()));
    }
    return outputFiles;
}
